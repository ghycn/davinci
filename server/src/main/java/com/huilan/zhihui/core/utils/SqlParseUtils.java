...
import com.huilan.zhihui.core.enums.SqlOperatorEnum;
import com.huilan.zhihui.core.enums.SqlVariableTypeEnum;
import com.huilan.zhihui.core.enums.SqlVariableValueTypeEnum;
import com.huilan.zhihui.core.model.SqlEntity;
import com.huilan.zhihui.model.SqlVariable;
import com.huilan.zhihui.model.SqlVariableChannel;
...
import static com.huilan.core.consts.Consts.*;
...
public class SqlParseUtils {
    ...
    public SqlEntity parseSql(String sqlStr, List<SqlVariable> variables, String sqlTempDelimiter) throws ServerException {
        ...
        char delimiter = Constants.getSqlTempDelimiter(sqlTempDelimiter);

        Pattern p = Pattern.compile(Constants.getReg(Constants.REG_SQL_PLACEHOLDER, delimiter, false));
    ...
    public String replaceParams(String sql, Map<String, Object> queryParamMap, Map<String, List<String>> authParamMap, String sqlTempDelimiter) {
        ...
        //替换auth@var
        Pattern p = Pattern.compile(Constants.getReg(Constants.REG_AUTHVAR, delimiter, true));
                        ...
                        if (!CollectionUtils.isEmpty(list)) {
                            ...
                            if (list.size() == 1) {
                                ...
                                if (!StringUtils.isEmpty(v)) {
                                    if (v.equals(Constants.N0_AUTH_PERMISSION)) {
                                        return "1=0";
                                    } else {
                                        switch (sqlOperator) {
                                            case IN:
                                ...
                                List<String> collect = list.stream().filter(s -> !s.contains(Constants.N0_AUTH_PERMISSION)).collect(Collectors.toList());
                                switch (sqlOperator) {
                                    case IN:
                                    case EQUALSTO:
                                        expBuilder
